...
public class Sendsms {
	...
	public static void sendSMS(String ph, String content) {

		String hostNameUrl = "https://sens.apigw.ntruss.com"; // 호스트 URL
		String requestUrl = "/sms/v2/services/"; // 요청 URL
		String requestUrlType = "/messages"; // 요청 URL
		String accessKey = "Ti1SduJQBDGFfr8eVP0g"; // 네이버 클라우드 플랫폼 회원에게 발급되는 개인 인증키 // Access Key :
													// https://www.ncloud.com/mypage/manage/info > 인증키 관리 > Access Key
													// ID
		String secretKey = "TWBmTKCg9XkcDcpR4JyYjmlDUGL0aPe1MT5QH4vG"; // 2차 인증을 위해 서비스마다 할당되는 service secret key //
																		// Service Key :
																		// https://www.ncloud.com/mypage/manage/info >
																		// 인증키 관리 > Access Key ID
		String serviceId = "ncp:sms:kr:289320688242:sens_test"; // 프로젝트에 할당된 SMS 서비스 ID // service ID :
		...
		toJson.addProperty("to", ph); // Mandatory(필수), messages.to 수신번호, -를 제외한 숫자만 입력 가능
		toArr.add(toJson);
		...
		bodyJson.addProperty("from", "01055412170"); // Mandatory, 발신번호, 사전 등록된 발신번호만 사용 가능
		// bodyJson.put("subject",""); // Optional, 기본 메시지 제목, LMS, MMS에서만 사용 가능
		bodyJson.addProperty("content", content); // Mandatory(필수), 기본 메시지 내용, SMS: 최대 80byte, LMS, MMS: 최대 2000byte
