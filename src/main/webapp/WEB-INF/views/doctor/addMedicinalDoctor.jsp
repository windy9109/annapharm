									...
									<table class="table table-bordered text-nowrap" style="table-layout: fixed;margin-bottom: 0px; width: 100%; overflow: auto; border: 0px;">
										<thead class="listTitle" style="position:sticky; top:0px">
											<tr>
												...
												<th style="padding: 10px 0px 10px 0px; width: 55px;">구분</th>
...
// 			 console.log(medicine);
			...
			var str = `<tr class="readyMedicineTr" val='\${mediNo}' ondblclick="deleteMedicineTr(this)">
					   		...
					   		<td>급여</td>
					   		<td style="text-align:center;"><input class="deleteCheck" name="deleteCheck" value="{{miCode}}" type="checkbox" style="width: 17px; height: 17px;"/></td>
					   	</tr>`;
...
<!-- 	</script> -->
	...
	<script type="text/javascript">
		...
		function addMedicineDoctor(){
			...
			$("tr.prescriptionDetail_tboy", window.opener.document).each(function(i,v){
				codeArr.push( $("td:eq(3)", $(v)).text() );
			});
...
// 					console.log(codeTd + "는 중복됨");
				...
				var ch = $("#prescriptionDetail_tboy", window.opener.document).childeNodes;
				//console.log('ch : ' + ch);
				
				var test = opener.document.getElementsByClassName('prescriptionDetail_tboy');
				//console.log('test : ',test.length);
				var test2 = opener.document.getElementsByClassName('prescriptionDetail_tboy')[test.length-1];
				...
				$("#prescriptionDetail", window.opener.document).append(`<tr class="prescriptionDetail_tboy" id="prescriptionDetail_tboy">
						<td>\${test3}</td>
						<td class="statusReady">\${classTd}</td>
						<td>\${typeTd}</td>
						<td class="miCode" value="\${mediNoTd}" >\${mediNoTd}</td>
						<td>\${useTd}</td>
						<td style="text-align: left;">\${nameTd}</td>
						<td>
							<input type="number" base-value="{{preDrugCount}}" class="preDrugCount" value="{{preDrugCount}}"  min="1" onchange="drugCount(this)" style="width: 35px;">
						</td>
						<td>
							<input type="number" base-value="{{preDrugDay}}" class="preDrugDay" value="{{preDrugDay}}" min="1" onchange="drugDay(this)" style="width: 35px;">
						</td>
						<td>급여</td>
						<td>
