...
<c:set var="annapharmInist" value="${dataMap.annapharmInList }" />
				...
				<div class="col-lg-12">
						<table class="table table-bordered">
							...
							<tbody>
								...
								<c:forEach items="${annapharmInList}" var="board">
								
								<tr>
									<td class="tableText">제목 ${board.chemist_in_title }</td>
									<td class="tableNum">답변수 ${board.chemist_in_reply_cnt }</td>
									<td>카테고리 ${board.chemist_in_type }</td>
									<td class="tableNum">조회수 ${board.chemist_in_viewcnt }</td>
									<td>최근답변일 ${board.chemist_in_date }</td>
									<td>작성일 ${board.chemist_in_date }</td>
								</tr>
								</c:forEach>
