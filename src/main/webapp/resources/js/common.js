...
function list_go(page,url){
	...
	jobForm.find("[name='searchType3']").val($('input[name="searchType3"]').val());
	jobForm.find("[name='searchType4']").val($('checked[name="searchType4"]').val());
	jobForm.find("[name='startDate']").val($('input[name="startDate"]').val());
	jobForm.find("[name='endDate']").val($('input[name="endDate"]').val());
	...
	target.summernote({
		...
		callbacks:{
			onImageUpload : function(files, editor, welEditable){
				...
				for(var file of files){
					sendFile(file,this); // 여기서 this는(current taget) textarea를 의미한다.
				}
			},
...
function deleteFile(src){
	...
	var fileData = {fileName:fileName}; // 데이터를 JSON화 해서 보내기
